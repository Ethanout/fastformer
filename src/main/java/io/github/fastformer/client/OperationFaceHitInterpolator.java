...
final class OperationFaceHitInterpolator {
   private static final double EPSILON = 1.0E-7;
   private final long transitionNanos;
   private OperationGeometry.RayHit value;
   private long updatedAt = Long.MIN_VALUE;

   OperationFaceHitInterpolator(long transitionNanos) {
      this.transitionNanos = Math.max(1L, transitionNanos);
   }

   OperationGeometry.RayHit update(OperationGeometry.RayHit target, long nowNanos) {
      if (target == null) {
         this.reset();
         return null;
      }
