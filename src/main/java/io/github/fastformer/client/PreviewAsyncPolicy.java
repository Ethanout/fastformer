...
final class PreviewAsyncPolicy {
   static final long SYNCHRONOUS_SCAN_LIMIT = 16_000L;
   static final int SYNCHRONOUS_GRID_LIMIT = 16_000;

   private PreviewAsyncPolicy() {
   }

   static boolean generateSynchronously(List<BlockPos> points, Workload workload) {
      return estimateScanCells(points, workload) <= SYNCHRONOUS_SCAN_LIMIT;
   }

   static boolean meshSynchronously(int blockCount) {
      return blockCount <= SYNCHRONOUS_GRID_LIMIT;
   }

   static long estimateScanCells(List<BlockPos> points, Workload workload) {
      if (points.isEmpty()) {
         return 0L;
      }
      if (points.size() == 1) {
