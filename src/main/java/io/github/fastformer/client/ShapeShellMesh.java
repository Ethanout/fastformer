...
final class ShapeShellMesh {
   ...
   record Part(List<AABB> boxes, Color faceColor, Color outlineColor, boolean outline) {
      Part {
         boxes = List.copyOf(boxes);
      }
   }

   record Mesh(List<Face> faces, List<StyledEdge> edges) {
      Mesh {
   ...
   record StyledEdge(Vec3 from, Vec3 to, Color color) {
   }
}
